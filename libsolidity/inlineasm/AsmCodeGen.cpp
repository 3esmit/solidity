...
public:
	...
	void operator()(assembly::Identifier const& _identifier)
	{
		...
		if (m_scope.lookup(_identifier.name, Scope::NonconstVisitor(
			[=](Scope::Variable& _var)
			{
				if (int heightDiff = variableHeightDiff(_var, _identifier.location))
...
private:
	void generateAssignment(assembly::Identifier const& _variableName, SourceLocation const& _location)
	{
		if (m_scope.lookup(_variableName.name, Scope::Visitor(
			...
			{
				if (int heightDiff = variableHeightDiff(_var, _location))
	...
	int variableHeightDiff(Scope::Variable const& _var, SourceLocation const& _location)
	{
		if (!_var.active)
		{
